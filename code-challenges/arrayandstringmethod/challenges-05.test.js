...
const listFoods = (recipe) => {
  ...
  for ( let i = 0; i < ingredients.length; i++){
      let each = ""
      let totalSpaces = 0
      for (let k = 0; k < ingredients[i].length;k++){
          if (ingredients[i].slice(k, k+1) === " "){
              totalSpaces = totalSpaces + 1;
              if (totalSpaces === 2) {
                  break;
              }
          } 
          each = ingredients[i].slice(k + 2, ingredients[i].length );
      }
      result.push(each)
  } 
  return result;
...
const splitFoods = (recipe) => {
  ...
  let newIngredient = split.slice(2);
  newIngredient = newIngredient.join(" ");
...
const removeLastCharacters = (str, numberOfCharacters) => {
  if (numberOfCharacters > 0){
      ...
      return str.slice(0, wordSize-numberOfCharacters)
  }
